...
/**
 * In pnpm workspaces, a workspace dependency may be linked as a "packed" subset
 * (respecting package.json "files"), and some folders (like mcp-tools) might
 * not appear under node_modules.
 *
 * Поэтому:
 * 1) пытаемся использовать node_modules (как раньше),
 * 2) если нет — используем исходники в монорепе packages/agent-core/mcp-tools.
 */
function resolveSkillsDir() {
  const candidates = [
    path.join(__dirname, '..', 'node_modules', '@accomplish_ai', 'agent-core', 'mcp-tools'),
    path.join(__dirname, '..', '..', '..', 'packages', 'agent-core', 'mcp-tools'),
  ];
  ...
  // Return the primary for nicer error messages
  return candidates[0];
}
...
const bundles = [
  { name: 'ask-user-question', entry: 'src/index.ts', outfile: 'dist/index.mjs' },
  { name: 'file-permission', entry: 'src/index.ts', outfile: 'dist/index.mjs' },
  { name: 'complete-task', entry: 'src/index.ts', outfile: 'dist/index.mjs' },
  { name: 'start-task', entry: 'src/index.ts', outfile: 'dist/index.mjs' },
  { name: 'dev-browser-mcp', entry: 'src/index.ts', outfile: 'dist/index.mjs', external: ['playwright'] },
  { name: 'dev-browser', entry: 'scripts/start-server.ts', outfile: 'dist/start-server.mjs', external: ['playwright'], banner: true },
  { name: 'dev-browser', entry: 'scripts/start-relay.ts', outfile: 'dist/start-relay.mjs', external: ['playwright'], banner: true },
];
...
async function main() {
  if (!fs.existsSync(skillsDir)) {
    ...
    throw new Error(`[bundle-skills] MCP tools directory not found. Tried:\n  - ${tried.join('\n  - ')}`);
  }

  validateSkillDependencyCategories();

  console.log(`[bundle-skills] Starting skill bundling (mode=${buildMode})...`);
  console.log(`[bundle-skills] Using MCP tools dir: ${skillsDir}`);
