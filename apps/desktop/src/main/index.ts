...
const APP_DATA_NAME = 'RodjerHelp';
app.setPath('userData', path.join(app.getPath('appData'), APP_DATA_NAME));

if (process.platform === 'win32') {
  app.setAppUserModelId('com.rodjer.rodjerhelp');
}
...
function buildRussianAppMenu() {
  // Windows/Linux menu labels are not localized by Electron by default.
  // We create a Russian menu explicitly.
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Telegram: @RodjerYan',
          click: async () => {
            await shell.openExternal('https://t.me/RodjerYan');
          },
        },
        {
    ...
    try {
      ...
      if (err instanceof FutureSchemaError) {
        await dialog.showMessageBox({
          ...
          message: `Эти данные созданы более новой версией Rodjer Help (schema v${err.storedVersion}).`,
          detail: `Ваше приложение поддерживает schema до v${err.appVersion}. Обновите Rodjer Help, чтобы продолжить.`,
          buttons: ['Выход'],
        });
        app.quit();
        return;
