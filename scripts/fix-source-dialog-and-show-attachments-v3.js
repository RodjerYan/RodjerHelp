/**
 * Fix v3: No backticks anywhere (even inside comments), to avoid breaking template strings.
 *
 * - Auto-answer "Источник" dialog (select option containing "Файл" then Submit) when an attachment exists.
 * - Auto-answer "Файл" dialog by inserting first attached file path and submitting.
 * - Make attachments visible in the sent user message by prepending "📎 Вложения: ..." and appending file contents
 *   between <<<FILE_CONTENT_START>>> and <<<FILE_CONTENT_END>>> markers.
 *
 * Usage:
 *   cd C:\Yandex.Disk\Project\RodjerHelp
 *   node scripts\fix-source-dialog-and-show-attachments-v3.js
...
function patchPermissionDialog() {
  ...
  if (!s.includes("AUTO_SOURCE_FILE_DIALOG_V3")) {
    const block = `
...
function patchHome() {
  ...
  if (!s.includes("ATTACHMENTS_PROMPT_V3")) {
    ...
    if (retIdx === -1) throw new Error("Cannot find return ( in Home.tsx");
...
try {
  ...
  patchHome();
  console.log("✅ Done. Rebuild now with: pnpm -F @accomplish/desktop build:unpack");
} catch (e) {
