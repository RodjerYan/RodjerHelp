 ...
 * 2) Attachment not visible in sent message -> prepend visible attachments header to prompt + provide file paths/contents.
 *
 * IMPORTANT: This v2 script avoids backticks inside template strings (which broke Node parsing).
 *
 * Usage:
 *   cd C:\Yandex.Disk\Project\RodjerHelp
 *   node scripts\fix-source-dialog-and-show-attachments-v2.js
        ...
        blocks =
          ...
          fileResults
            .map((f:any) => {
              ...
              // Avoid ``` fences to keep this script valid: use markers instead
              return head + '\\n\\n<<<FILE_CONTENT_START>>>\\n' + (f.text ?? '') + '\\n<<<FILE_CONTENT_END>>>';
            })
            .join('\\n\\n');
      } catch {}
...
`;
  ...
  // Wrap prompt in task:start call
